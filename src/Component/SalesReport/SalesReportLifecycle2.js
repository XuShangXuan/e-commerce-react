...
import ManagerHeader from '../BackEnd/BackEndHeader';
...
class SalesReportLifecycle2 extends Component {
    ...
    componentDidMount() {
        ...
        // 監聽整個JSX有submit事件被按下時，執行這行code，呼叫this.updateNewDate這個function，而且不會往下執行this.searchSalesReport(1)這行code
        window.addEventListener('submit', this.updateNewDate);
    ...
    */
    ...
    // 卸載階段當使用者離開這個component時，就執行裡面的code
    componentWillUnmount() {
        console.log("5.componentWillUnmount 組件卸載(Unmounting:卸載)");
        // 移除監聽，避免佔用電腦資源
        window.removeEventListener('submit', this.revertCurrentPage);
    }
        ...
        return (
            <div>
                ...
                {goodsReportSalesList.length > 0 &&
                    <div>
                        <button onClick={() => this.onClickPage(1)} disabled={genericPageable.currentPage == 1}>{'<<'}</button>
                        <button onClick={() => this.onClickPage(genericPageable.currentPage - 1)} disabled={genericPageable.currentPage == 1}>{'<'}</button>
                        {genericPageable.pageRange.map((p, index) => (
                            p == genericPageable.currentPage ? (
                                <button key={index} onClick={() => this.onClickPage(p)}><u><b>{p}</b></u></button>
                            ) : (
                                <button key={index} onClick={() => this.onClickPage(p)}>{p}</button>
                            )
                        )
                        )}
                        <button onClick={() => this.onClickPage(genericPageable.currentPage + 1)} disabled={genericPageable.pageTotalCount == genericPageable.currentPage}>{'>'}</button>
                        <button onClick={() => this.onClickPage(genericPageable.pageTotalCount)} disabled={genericPageable.pageTotalCount == genericPageable.currentPage}>{'>>'}</button>
                    </div>
                }
            </div>
        );
...
export default SalesReportLifecycle2;